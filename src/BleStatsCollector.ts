...
class TimestampRing {
  ...
  push(timestampMs: number): void {
    this.values[this.nextIndex] = timestampMs;
    this.nextIndex = (this.nextIndex + 1) % this.values.length;
    this.count = Math.min(this.count + 1, this.values.length);
  }

  countSince(cutoffMs: number): number {
    let result = 0;
    const startIndex = this.count < this.values.length ? 0 : this.nextIndex;
    for (let index = 0; index < this.count; index += 1) {
      const sourceIndex = (startIndex + index) % this.values.length;
      if ((this.values[sourceIndex] ?? 0) >= cutoffMs) {
        result += 1;
      }
    }
    return result;
...
export class BleStatsCollector {
  ...
  private channelSnapshot(
    ...
    return {
      ...
      recentRateHz:
        (this.recentChannelTimesMs[id]?.countSince(nowMs - RECENT_RATE_WINDOW_MS) ?? 0) /
        recentWindowSeconds,
      expectedRateHz,
  ...
  liveChannelsSnapshot(nowMs: number): ChannelLiveSnapshot[] {
    if (this.startedAtMs === 0) {
      return [];
    }
    ...
    const cutoffMs = nowMs - RECENT_RATE_WINDOW_MS;
    const result: ChannelLiveSnapshot[] = [];
    ...
    for (let id = 0; id < this.channelCounts.length; id += 1) {
      ...
      result.push({
        ...
        averageRateHz: count / elapsedSeconds,
        recentRateHz:
          (this.recentChannelTimesMs[id]?.countSince(cutoffMs) ?? 0) /
          recentWindowSeconds,
        latestRaw: this.channelHasRaw[id] === 1 ? this.channelLatestRaw[id] ?? 0 : null,
        lastSeenAgoMs:
          this.channelHasRaw[id] === 1
            ? Math.max(0, nowMs - (this.channelLastSeenMs[id] ?? nowMs))
            : null,
